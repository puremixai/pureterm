"""驱动 Windows 桌面应用的极简工具（纯 ctypes，不需要 Add-Type / 任何安装）。

为什么不用别的路子：
  - PowerShell + Add-Type 被安全策略挡着（"compiles and loads .NET code at runtime"）
  - computer-use 技能的 sky 运行时是 Codex 专有的，WorkBuddy 里跑不了
  - ctypes 是 Python 标准库，直接调 user32，什么都不用装
