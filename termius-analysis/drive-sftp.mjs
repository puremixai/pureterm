/**
 * ssh-cordis-client 的桌面端端到端验证：远端文件（SFTP）。
 *
 * 为什么不塞进 `test/smoke-*.mjs`：这条路上有**三个原生模态窗口**——
...
import electronPath from '../ssh-cordis-client/node_modules/electron/index.js'
import { startFakeSshServer } from '../ssh-cordis-client/test/fake-ssh-server.mjs'

const HERE = dirname(fileURLToPath(import.meta.url))
const ROOT = join(HERE, '..', 'ssh-cordis-client')
const SHOTS = join(HERE, 'shots')
const CAPTURE = join(HERE, 'capture.cjs')
const CROP = join(HERE, 'crop.py')
const DRIVE = join(HERE, 'drive.py')
const PYTHON = 'C:/Users/ausu/.workbuddy/binaries/python/versions/3.13.12/python.exe'
const TITLE = 'SSH Cordis Client'
...
async function main() {
  ...
  child = spawn(electronPath, [join(ROOT, 'dist', 'electron', 'main.js'), `--remote-debugging-port=${DEBUG_PORT}`], {
    env,
