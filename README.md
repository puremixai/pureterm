...
- Each session can report the remote host's resources. The row under the session toolbar starts **collapsed**; expanding it begins collection and shows CPU, memory, load, the root filesystem's usage, the network rate and the host's uptime, with a pause and a retry beside them. Collapsing the row, pausing it, hiding the page, switching tabs, or disconnecting all stop collection — a collapsed session costs the remote nothing. The status bar's cipher and host-key cells are filled from the SSH handshake rather than from this row, so they stay filled even when the row is collapsed.
