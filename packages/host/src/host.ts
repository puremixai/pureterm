...
export { sessionOnlyCredentials, type CredentialProvider } from './credentials.js'
...
export async function createHost(options: HostOptions): Promise<Host> {
  ...
  const storeConfig = { file: options.hostStoreFile, secretsFile, credentials: options.credentials ?? sessionOnlyCredentials }
  assertSessionStoreCompatible(storeConfig)
