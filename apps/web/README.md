...
Each session can also report the remote host's resources over the same WebSocket and the same SSH connection as the terminal. The row under the session toolbar is **collapsed by default**; expanding it starts a five-second Linux probe and shows CPU, memory, load, the root filesystem's usage, the network rate and the host's uptime, with pause and retry beside them. Collapsing, pausing, hiding the page, switching tabs, and disconnecting all stop collection. CPU and network are rates and stay empty until a second probe supplies a baseline; disk is the root filesystem only; and a remote that is not Linux is reported as unsupported rather than as a failure. The status bar's cipher and host-key cells come from the SSH handshake rather than from this row, so they stay filled while it is collapsed.
...
The last command uses a test Chromium window to access the standalone Node service. It covers a WebSocket terminal without preload, real SSH authentication with both a directly selected browser key and a Keychain key reference, terminal echo, credential/key-association clearing after refresh, and a fixture resource snapshot with its session facts rendered through the real UI on a connection that still carries terminal and SFTP traffic. Electron is used only as the test browser; the Web service itself is an ordinary Node process.
