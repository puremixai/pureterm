...
import type { HostSaveRequest } from '../../shared/protocol.js'
 ...
 * 2. 密码用 Electron safeStorage（Windows DPAPI / macOS Keychain）加密后落盘；
 *    解出来的明文只在本进程内用于连接，绝不回传渲染层（HostRecord 里只有 hasSecret）。
 */
export class SessionStore extends Service {
  static inject = ['renderer']
  ...
  private toRecord(host: StoredHost): HostRecord {
    ...
    return { ...rest, hasSecret: !!this.secrets[host.id] }
  }
  ...
  save(input: HostInput): HostRecord {
    ...
    record.privateKeyPath = input.privateKeyPath ?? record.privateKeyPath
    ...
    if (input.rememberPassword && credential) {
      const sealed = this.ctx.renderer.seal(credential)
  ...
  secret(id: string): string | undefined {
    ...
    return this.ctx.renderer.unseal(sealed)
  }
}
