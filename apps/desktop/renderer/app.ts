...
import { MAX_TRANSFER_BYTES, type HostRecord, type HostSaveRequest, type SftpDir, type SftpEntry, type TerminalOpenRequest } from '../shared/protocol.js'
...
function syncAuthRows(): void {
  ...
  rememberLabel.textContent = method === 'privateKey' ? '记住口令' : '记住密码'
...
function applyHost(record: HostRecord): void {
  ...
  keyPathInput.value = record.privateKeyPath ?? ''
  passInput.value = ''
  keyPassInput.value = ''
...
async function saveCurrentHost(): Promise<HostRecord | null> {
  ...
  const record = await api.hosts.save({
    ...
    username,
    authMethod: method,
    // 私钥**只存路径**：内容在连接那一刻由主进程现读，不进我们的数据目录
    privateKeyPath: method === 'privateKey' ? keyPathInput.value.trim() || undefined : undefined,
    password: method === 'password' ? passInput.value || undefined : undefined,
    passphrase: method === 'privateKey' ? keyPassInput.value || undefined : undefined,
    rememberPassword: rememberCheck.checked,
  })
...
async function connect(): Promise<void> {
  ...
  const payload: TerminalOpenRequest = {
    ...
    username,
    authMethod: method,
    // hostId 是「去哪个密文槽取凭据」的入口。不填就只能用这次请求里带来的明文凭据。
  ...
  if (method === 'privateKey') {
    payload.privateKeyPath = privateKeyPath
    if (keyPassInput.value) payload.passphrase = keyPassInput.value
  } else if (password) {
  ...
  try {
    ...
    if (rememberCheck.checked && credentialFilled) await saveCurrentHost()
  } catch (error) {
