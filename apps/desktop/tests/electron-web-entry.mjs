...
import { createCompositeBridge } from '../dist/electron/carriers/carrier.js'
...
try {
  ...
  host = await createHost({
    ...
    bridge: createCompositeBridge(() => carriers, { seal: () => undefined, unseal: () => undefined }),
  })
  let acceptReady
  let rejectReady
  ...
  void ready.catch(() => {})
  const dispatcher = createDispatcher({ host, pickPrivateKey: async () => undefined, onReady: (payload, clientId) => {
  ...
  carrier = await createHttpCarrier({ dispatcher, staticDir: fileURLToPath(new URL('../dist/renderer', import.meta.url)) })
