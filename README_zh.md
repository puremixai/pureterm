...
- 每个会话可以报告远端主机的资源。会话工具栏下面那一行**默认折叠**；展开后才开始采集，显示 CPU、内存、负载、根文件系统占用、网络速率和主机运行时间，右侧带暂停和重试。折叠、手动暂停、页面不可见、切走标签页、断开连接都会停止采集——折叠的会话对远端零开销。状态栏的 cipher 与 host key 两格来自 SSH 握手而不是这一行，所以折叠时它们照样填着。
