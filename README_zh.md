...
Desktop 支持系统加密保存密码或私钥口令、原生私钥文件选择。独立 Web 保存主机信息，密码、口令和浏览器选中的私钥仅用于当前页面，刷新后需要重新输入或选择。详情见 [Desktop 说明](apps/desktop/README_zh.md)与[本机 Web 说明](apps/web/README_zh.md)。
...
- `+` 或 Ctrl/Cmd+T 新建连接，Ctrl+Tab / Ctrl+Shift+Tab 切换标签，Ctrl/Cmd+W 关闭当前终端标签。聚焦标签按钮后也可使用方向键、Home、End。浏览器可能占用部分快捷键，仍可使用页面上的按钮。

打开的会话与重试凭据只保留在当前客户端，刷新后不会恢复。SFTP 目前支持目录浏览、上传/下载（单文件最多 4 MiB）、新建文件夹及确认后删除。串口、端口转发、独立密钥库和命令片段管理尚未实现，不再作为可用控件显示。
