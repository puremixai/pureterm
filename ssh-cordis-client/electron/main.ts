...
import { createBootCheck } from './boot-check.js'
import { createCompositeBridge, type Carrier, type Credentials } from './carrier.js'
import { createHttpCarrier } from './carrier-http.js'
import { createIpcCarrier, ipcClientId } from './carrier-ipc.js'
...
import { collectSwitches } from './platform-plan.js'
...
import { relaunchSelf } from './relaunch.js'
 ...
 * 两个同时活着，共用**同一个** dispatcher（electron/dispatch.ts）和同一棵插件树。
...
/** 平台差异在这里选一次，之后全项目只读（electron/platform-plan.ts 里有决策的纯逻辑） */
    ...
    onFailure: (error) => {
      console.error(
        [
          ...
          '[main]   electron dist/electron/main.js --no-sandbox',
        ].join('\n'),
      )
    },
  })
