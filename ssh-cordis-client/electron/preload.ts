...
// 无扩展名：本文件由 esbuild 单独打包成 CJS（见 scripts/build-preload.mjs），
// 不走 Node ESM 的解析规则，所以不受「相对导入必须带 .js」的约束。
import { EVENTS, METHODS, NOTICES, type PickedPrivateKey, type SshApi } from '../shared/protocol'

/**
 * 这个文件被编译成 dist/electron/preload.cjs（CJS）。
