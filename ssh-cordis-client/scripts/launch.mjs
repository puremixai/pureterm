 ...
 *   - 「这台机器需不需要放宽沙箱」是应用自己的判断（electron/platform-plan.ts 负责
 *     运行时决策，启动档案 electron/launch-profile.ts 负责记住上次的结果）；
 ...
 * 哪套配置。真正的回填由应用在创建窗口之前完成（electron/main.ts 的「启动决策」段）。
 *
 * 用法：
...
const mainScript = join(projectRoot, 'dist', 'electron', 'main.js')
...
/** 与 electron/main.ts 保持一致：环境变量优先，否则 ~/.ssh-cordis */
...
const { readLaunchProfile, planProfileSwitches, describeLaunchProfile, launchProfilePath } = await import(
  new URL('../dist/electron/launch-profile.js', import.meta.url).href
)
...
const child = spawn(electronPath, ['dist/electron/main.js', ...extra], {
  cwd: projectRoot,
  env,
  detached: true,
