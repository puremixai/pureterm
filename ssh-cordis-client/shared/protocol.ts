 ...
 * 渲染层 ⇄ 壳：**唯一的**协议定义处。
 *
 * 为什么必须单独一个文件：以前通道名在 `electron/main.ts`（`ipcMain.handle('ssh:open', …)`）
 * 和 `electron/preload.ts`（`ipcRenderer.invoke('ssh:open')`）里各写了一遍字面量。
 ...
 * 而 WebSocket 载体单条报文的上限是 8 MiB（`electron/ws-frame.ts` 的 MAX_MESSAGE_BYTES）。
