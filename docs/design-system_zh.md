...
在共享样式表**内部**，颜色字面量只允许出现在两个文件里。`packages/ui/src/styles/tokens.css` 是体系本身：中性明度阶梯、文字阶梯、强调色与语义色集、终端组，以及与主题无关的度量。`packages/ui/src/styles/legacy.css` 是唯一被认可的例外 —— 改版前的色板被作为一份受守卫的删除清单保存在那里，而不是放任它们重新散布回各分片。其余分片的颜色一律透过 token 解析。

这个限定语是承重的，因为这条规则说的是 CSS，而守卫读取的是分片。级联之外有两处持有颜色字面量：`packages/ui/src/terminal-view.ts` 里那份十六项的 `ANSI` 数组，这是刻意的，见[终端色板](#终端色板)；以及 `packages/ui/src/index.html:15` 的 `<meta name="theme-color" content="#121426">`，一个没有守卫读取的退役取值，记录在[遗留登记簿](#遗留登记簿)。把它们点明在这里，才让"两个文件"这句话既成立又不越界。
...
- 任何分片里的每一个 `var(--x)` —— 两个登记表本身也不例外 —— 都必须指向某个登记表声明过的 token。这是登记表自身那条存活检查的反方向，也是配色翻转期间最要紧的一条：未知自定义属性并不是错误，它在计算值阶段被替换掉，于是一个写错的 `var(--fs-metax)`（在分片里）或 `var(--c-canvasx)`（在某个 token 取值里）会让那条声明悄悄从页面上消失。
...
| 2 | `styles/tokens.css` | 3,887 | Token 体系与主题组。 |
| 3 | `styles/legacy.css` | 6,165 | 作为债务登记簿的改版前色板。 |
...
这一列字节取自 Windows 上的工作副本，并且刻意这样标注，因为它并非到处都能复现：`core.autocrlf` 为 `true`，仓库里又没有 `.gitattributes`，于是提交的 blob 是 LF，而这里十个文件里有八个在磁盘上是 CRLF。保留 blob 形态的检出会读到每个这样的文件恰好少它的行数 —— `tokens.css` 123、`legacy.css` 136、`base.css` 71、`chrome.css` 186、`hosts.css` 90、`inspector.css` 86、`keychain.css` 70、`states.css` 42 —— 而 `fonts.css` 与 `terminal.css` 本身已按 LF 存储，两处读数一致。把这些数字当作每个职责承载了多少的一个量级参考，不要当作校验和。
...
这些数值逼出的规则是：**在浅色主题里，弱化文字与强调色文字只属于 `--c-surface`。** `--tx-3` 与 `--ac` 在那里越过 4.5:1（4.83 与 4.63），到了 `--c-control` 与 `--c-chrome` 就跌到 3.88–4.23。深色组宽松得多，但也不是无条件：`--ac` 在每个深色底面上都过关（最差 6.55:1，在 `--c-control` 上），而 `--tx-3` 自己会在 `--c-raised` 上落到 4.48、在 `--c-control` 上落到 4.03。`tokens.css` 里压在文字阶梯之上那段注释说的是同一个限度，因此源码与本表在此一致。
...
细线被有意排除在这套纪律之外：它们不是文字。`--line` 在深色组的 `--c-surface` 上测得 1.27:1（浅色组 1.29:1），而这个接近 1.27:1 的目标是刻意的 —— 用一道几乎察觉不到的边来承载深度，正如上面的阶梯在相邻两级之间只以 1.04–1.19 承载深度。被它替换掉的 `rgba(222, 226, 255, 0.085)` 在当前描边的六个遗留底色上合成后是 1.21:1–1.26:1（`--topbar` 1.21、`--nav` 1.26、`--main` 1.25、`--main-soft` 1.25、`--card` 1.26、`--field` 1.23），所以新 token 只是略微更显眼，而不是对分隔线的重新设计。让一根细线去通过文字对比度测试才是这段说明要防的错误。
...
- 登记簿里七道活着的细线如今收在同一组阶梯注释下，由弱到强：`--legacy-line-faint` .05、`--legacy-line-hair` .07、`--legacy-line-edge` .08、`--legacy-line` .085、`--legacy-line-mid` .12、`--legacy-line-field` .13、`--legacy-line-strong` .17。新阶梯的三道 —— `--line-soft`、`--line`、`--line-strong` —— 仍待接线，各自引用为零。于是 `--line-strong` 与 `--legacy-line-strong` 是两个平行名字对应着两个不同取值，一活一待，翻转时一次盲目的 `s/--legacy-//` 就会把活的 alpha 落到那个不透明台阶该在的位置。请按实测取值配对，绝按名字配对；这个陷阱在 `legacy.css` 自己的头部注释里也点了名。
