...
- every `var(--x)` in any partial, the register included, must name a token `tokens.css` declares — the test is `every var() reference names a token the register declares`. This is the opposite direction to a declaration's own liveness check, and it is the one that mattered during the flip: an unknown custom property is not an error, it is substituted at computed-value time, so a mistyped `var(--fs-metax)` in a partial or a `var(--c-canvasx)` inside a token value makes the declaration quietly stop existing on the page. The palette flip wrote some 300 of the two manifests' current 524 call sites in a single plan, which is the whole exposure created in one commit and the whole reason the guard exists.
...
| 2 | `styles/tokens.css` | 6,227 | The token system and the theme groups. |
| 3 | `styles/base.css` | 4,579 | Element defaults: reset, inherited type, buttons and inputs, focus rings, reduced motion. |
| 4 | `styles/chrome.css` | 12,727 | Application shell: the `#app` grid, top bar, workspace and session tabs, navigation rail, status bar, and the state classes on `.app-shell`. |
| 5 | `styles/hosts.css` | 13,064 | Hosts dashboard: page header, search row, toolbar, and the table and card shapes a saved host renders in. |
| 6 | `styles/inspector.css` | 10,314 | Connection form: the docked editor shell and its header, sections, fields and footer, plus the shared controls the keychain editor reuses. |
| 7 | `styles/keychain.css` | 10,464 | Keychain library and editor over the shared host-table language. |
| 8 | `styles/terminal.css` | 14,807 | Terminal surface: the xterm pane and its overrides, the session toolbar, the session resource row, the SFTP drawer. |
| 9 | `styles/states.css` | 12,617 | Nothing-to-show and something-went-wrong: empty placeholder, shortcuts dialog, connection-failure page. |

`desktop.css` carries one partial, `styles/window-controls.css` — 2,001 bytes on disk and 38 lines, holding the top bar's minimize/maximize/close rules. The split is the point rather than an accident of file size: `index.html` and the built `app.css` are what both entry points share, and the Web entry has no window to control, so it must carry neither the markup nor the rules. The markup is not in `index.html` at all — `services/chrome.ts` links `./desktop.css` and builds the three buttons, and only when `window.puretermDesktop.windowControls` is present. Which platforms that is, is a platform question with one answer (`drawsOwnWindowControls` in the Desktop's platform plan): Windows and Linux, whose `titleBarStyle: 'hidden'` leaves no native buttons, but not macOS, which keeps its own traffic lights and must not have a second set drawn beside them. Measured on the built sheets, `app.css` contains no rule whose selector mentions `.window-control`, and every rule in `desktop.css` is one of them.

The session resource row is the one place in the session screen that changes height on purpose, and the reason its default is collapsed is a height budget rather than taste. Collapsed it is a 28px control row with no ground and no border of its own, so the screen keeps the three fixed bands it already drew — the 40px top bar, the 40px session toolbar and the 24px status bar — and the terminal gives up nothing. Expanded it becomes a 40px band with the toolbar's `--line` hairline and `--c-chrome` ground, and the terminal is the only cell that absorbs the 12px, because `.session-content` is the flexible `min-height: 0` track and the row is `flex: 0 0 auto`. Below 820px the row wraps to a second line instead of overflowing. Status is carried by text and only repeated by colour, so the row still reads under a colour-vision difference. These are measured, not asserted from intent: the row is 28px collapsed and 40px expanded, the terminal loses exactly the 12px difference, and both widths hold `scrollWidth === innerWidth`.

The size column is a working-tree measurement taken on Windows, and it is labelled that way because it does not reproduce everywhere: `core.autocrlf` is `true` and there is no `.gitattributes`, so the committed blobs are LF while eight of these ten files carry CRLF on disk. A checkout that keeps the blob form reads each of those eight smaller by exactly its line count — `tokens.css` 175, `base.css` 93, `chrome.css` 233, `hosts.css` 146, `inspector.css` 116, `keychain.css` 121, `states.css` 128, `window-controls.css` 38 — while `fonts.css` and `terminal.css` are stored with LF endings already and read identically. `style.css` itself is CRLF and 1,363 bytes on disk against a 1,340-byte blob, and `desktop.css` is CRLF and 765 bytes against a 755-byte blob. Treat the numbers as an indication of how much each role carries, not as a checksum.
...
`--r-4` and `--ease` are the two that arrived late and for a specific reason: they were `--radius-lg: 15px` and `--motion-standard: cubic-bezier(0.32, 0.72, 0, 1)` in the deleted register, non-colour entries with 22 live references that `git rm legacy.css` would otherwise have taken down with them. All four radius and motion entries were re-homed before the file went, onto the nearest step of the new ladder — 7px to `--r-2`, 10px to `--r-3`, 15px to `--r-4`, and the curve to `--ease` — each of which is a deliberate, visible change rather than a value-preserving move: the new steps are tighter, and `--ease` settles faster while ending at the same 1. After the alignment the ladder carries 50 references — `--r-1` 22, `--r-2` 21, `--r-3` 4, `--r-full` 3 — and `--ease` 16, which is what a register-free geometry looks like. `--r-4` now has **none**, and that is the alignment's doing rather than an oversight: the prototype never draws a 12px corner, so the step is retired the way `--z-drawer` and `--t-1` already are, and `the control language is the prototype's 28px scale` in `visual-contract.test.mjs` fails on any declaration that reaches for it again.

The geometry steps are the reason a shell height is written once. `--chrome-h` is the top bar's height and the first row of the `#app` grid, and `theme-sync.test.mjs` reads it from this file and asserts it against both places `chrome.css` spends it — the grid template and the `.app-topbar` height — so the number is a layout contract rather than a coincidence that happens to look right. Nothing else reserves space at the top any more: the desktop window runs `titleBarStyle: 'hidden'` with no `titleBarOverlay`, and the same test asserts both halves of that, because a system overlay reserving a strip beside the self-drawn buttons in `window-controls.css` would be two sets of caption controls on one bar. That is also why the `env(titlebar-area-height, 40px)` fallback this paragraph used to describe is gone: it existed to keep the reserved strip and the token the same, and with the overlay removed there is no strip to keep in step.
...
`[data-density="compact"]` is an orthogonal dimension to the theme and remaps one token: `--row-h: var(--row-h-compact)`. For that remap to reach a library row, the row's own button must not floor it. `base.css` gives every `button` a `min-height: 28px` — the prototype's control scale — which now sits below both `--row-h` (38px) and `--row-h-compact` (30px), so the row's box is the target and the button merely fills it. The three `min-height: 0` overrides that were load-bearing when the floor was the comfortable row height — `#host-list:not(.card-view) .host-main`, `.keychain-list:not(.card-view) .keychain-card-main` and `.file-row .file-main` — stay anyway: `theme-sync.test.mjs` and `visual-contract.test.mjs` assert each one by name, and the comment beside each says the guard is the reason it remains.

### Type scale

| Token | Value | Consumers today |
| --- | --- | --- |
| `--font-ui` | `Inter, "Segoe UI Variable Text", "Segoe UI", "Microsoft YaHei UI", system-ui, sans-serif` | 2 `var()`: `body` in `base.css`, the `kbd` rule in `states.css` |
| `--font-mono` | `"JetBrains Mono", "Cascadia Mono", Consolas, monospace` | 16 `var()` across `base`, `chrome`, `hosts`, `inspector`, `states`, `terminal` |
| `--font-term` | `"Cascadia Mono", Consolas, "Sarasa Mono SC", "Microsoft YaHei Mono", monospace` | 1 `read()` in `terminal-view.ts` |
| `--fs-micro` | `11px` | 34 `var()` |
| `--fs-meta` | `12px` | 30 `var()` |
| `--fs-ui` | `13px` | 5 `var()` |
| `--fs-em` | `14px` | 2 `var()` |
| `--fs-h2` | `16px` | 2 `var()` |
...
This is a census rather than a promise: the eight content partials hold 521 `var()` references, `tokens.css` holds 3 more inside its own derived values, and `terminal-view.ts` makes 6 `read()` calls. What is wired and what the screens still owe:
...
- **Metrics: 79 references across the five radius steps, `--ease`, the four chrome steps and `--row-h`, and eight steps with none** — the radius ladder carries 50 (`--r-1` 22, `--r-2` 21, `--r-3` 4, `--r-full` 3, `--r-4` none), `--ease` 16, the chrome geometry 10 (`--chrome-h` 3, `--rail-w` 1, `--status-h` 3, `--insp-w` 3) and `--row-h` 3. `--r-4` went to none in the prototype alignment — no screen draws a 12px corner — and the guard `the control language is the prototype's 28px scale` in `visual-contract.test.mjs` fails on any declaration that reaches for it again. The eight unused steps are three of the six spacing steps (`--s-2` is drawn by the top bar's action cluster and by the empty state, `--s-3` and `--s-4` by the toast stack), three of the four z-index stops (`--z-toast` is live; `--z-drawer`, `--z-popover` and `--z-dialog` are not) and two of the three durations. `--row-h` gained call sites when both tables adopted it as their row height, and it became able to *move* one when the row's own button stopped flooring it at its comfortable value; the density switch changed a token and no geometry before that. `--row-h-compact` still has no consumer beyond the `[data-density="compact"]` rule that maps onto it. The durations were once the one place where "no consumer" was not neutral: the partials set them, and set them as literals — `180ms` in four files and `160ms` in a fifth — so the curve was tokenised while the number beside it stood off the ladder, 20ms from a rung. That is closed: every duration in the cascade now reads `--t-2`, which is the rung 180ms was 20ms away from, so `--t-1` and `--t-3` are now honestly unused rather than quietly contradicted.
- **Derived: `--ring` has three call sites** (`base.css` for the focus ring, `chrome.css` for the nav item, `terminal.css` for the session grip) **and `--shadow-pop` two** (the shortcuts dialog and the toast), which is thin but not zero, and both are measured by the pairs they are built from.
...
Four weights, no more: the census allows 400, 500, 600 and 700 (and the `normal`/`bold` keywords). Today the 23 weight declarations in the partials sit 400 five times, 500 eleven times, 600 six times and 700 once — the lone 700 is the two-letter host monogram in `.host-avatar`. Inter is variable and can render 650 or 750, and that is exactly the problem: the CJK and system fallbacks in `--font-ui` cannot, so an off-scale value asks a fallback for a weight it does not own and the browser synthesises one. Synthetic bold on 13px UI text is the smear this list prevents. Before this change the partials did use 650, 750 and 800; the palette flip moved one 700 label down to 500, and the prototype alignment then moved the rest onto the prototype's 500-and-600 scale, which is what a token system with a legal weight list makes a deliberate decision instead of an accident.
...
- `#keychain-fingerprint` — the only `<code>` element in the page (`index.html:134`) — has no `font-family` rule. `keychain.css:81` sizes and colours it and stops there, so a key fingerprint renders in the user agent's default monospace rather than in `--font-mono`. The family census cannot see this: it rejects a declaration that names a stack instead of a token, and an absent declaration is not a declaration.
...
- Twelve tokens are still unreachable by anything on screen: `--overlay-soft`, `--overlay-press` and `--ac-focus` among the colours, `--r-4` among the radii (retired by the prototype alignment, which draws no 12px corner), and among the metrics three spacing steps (`--s-1`, `--s-5`, `--s-6`), three z-index stops (`--z-drawer`, `--z-popover`, `--z-dialog`) and two durations (`--t-1`, `--t-3`). They are verified by `design-tokens.test.mjs` in isolation rather than by a rendered pixel, which is the intended state of a ladder waiting for the screens — but it means a mistake in one of those twelve values is a mistake the browser has not yet contradicted.
...
- Below 620px both docked editors escape their track. `.keychain-editor` and `.connection-workspace` go `position: absolute; inset: …` at 900px, which is exact while `#main` has a definite height from the `.app-body` grid — but the 620px block turns `.app-body` into a scrolling flex column, so `#main`'s height becomes content-driven and the containing block an absolutely-positioned editor resolves against is the list behind it rather than the viewport. Neither editor has been measured below 1080px: the in-app viewport is fixed, and the loopback host refuses to be framed. Recorded rather than fixed blind.
