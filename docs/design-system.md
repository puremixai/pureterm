...
| 4 | `styles/chrome.css` | 8,927 | Application shell: the `#app` grid, top bar, workspace and session tabs, navigation rail, status bar, and the state classes on `.app-shell`. |
| 5 | `styles/hosts.css` | 10,765 | Hosts dashboard: page header, search row, toolbar, and the table and card shapes a saved host renders in. |
| 6 | `styles/inspector.css` | 7,730 | Connection form: the docked editor shell and its header, sections, fields and footer, plus the shared controls the keychain editor reuses. |
| 7 | `styles/keychain.css` | 9,052 | Keychain library and editor over the shared host-table language. |
...
This is a census rather than a promise: the seven content partials hold 381 `var()` references, `tokens.css` holds 3 more inside its own derived values, and `terminal-view.ts` makes 6 `read()` calls. What is wired and what the screens still owe:
