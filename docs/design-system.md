...
- every `var(--x)` in any partial, the register included, must name a token `tokens.css` declares — the test is `every var() reference names a token the register declares`. This is the opposite direction to a declaration's own liveness check, and it is the one that mattered during the flip: an unknown custom property is not an error, it is substituted at computed-value time, so a mistyped `var(--fs-metax)` in a partial or a `var(--c-canvasx)` inside a token value makes the declaration quietly stop existing on the page. 299 of the cascade's call sites were written by that one plan, which is the whole exposure created in one commit and the whole reason the guard exists.
...
| 2 | `styles/tokens.css` | 5,598 | The token system and the theme groups. |
| 3 | `styles/base.css` | 3,004 | Element defaults: reset, inherited type, buttons and inputs, focus rings, reduced motion. |
| 4 | `styles/chrome.css` | 8,679 | Application shell: the `#app` grid, top bar, workspace and session tabs, navigation rail, and the state classes on `.app-shell`. |
...
The palette flip is done, so this is the census rather than the promise: the seven content partials hold 354 `var()` references, `tokens.css` holds 3 more inside its own derived values, and `terminal-view.ts` makes 6 `read()` calls. What the flip consumed and what it left for the chrome rebuild:

- **Colours: 32 of 35 have a call site.** The three that do not are `--overlay-soft`, `--overlay-press` and `--ac-focus` — a resting lift, a pressed lift, and the focus ring's second stop, all of which need the structural work in the next plan before there is a control to put them on.
...
- **Terminal: 1 CSS consumer, 4 TS reads.** `--term-bg` is now painted by `terminal.css:6` as well as read by xterm, which is the seam the comment above that rule explains; the other three reach the page only through `getComputedStyle`, so no CSS guard can see them and `terminal-theme.test.mjs` is the tie.
- **Metrics: 56 references across the four radii and `--ease`, and 14 tokens with none** — `--r-full`, the six spacing steps, the four z-index stops and the three durations. These are the ladder the chrome rebuild writes against. Both row heights sit between the two categories: `--row-h` and `--row-h-compact` have no partial call site and currently reference only each other, through the `[data-density="compact"]` rule. The motion pair is the one place where "no consumer" is not neutral: the partials do set durations, and they set them as literals — `180ms` in `base.css`, `hosts.css`, `inspector.css` and `terminal.css`, `160ms` in `keychain.css` — so the curve is tokenised while the number beside it is off the ladder, 20ms away from `--t-2`.
- **Derived: `--ring` and `--shadow-pop` have one call site each** (`base.css` for focus, `states.css` for the failure dialog's popover), which is thin but not zero, and both are measured by the pairs they are built from.
...
- Seventeen tokens are still unreachable by anything on screen: `--overlay-soft`, `--overlay-press` and `--ac-focus` among the colours, and `--r-full`, the six spacing steps, the four z-index stops and the three durations among the metrics. They are verified by `design-tokens.test.mjs` in isolation rather than by a rendered pixel, which is the intended state of a ladder waiting for the chrome rebuild — but it means a mistake in one of those seventeen values is a mistake the browser has not yet contradicted.
- The light theme has no switch, so no cell of the light columns in this document is reachable through the product. See [Theme mechanism](#theme-mechanism).
