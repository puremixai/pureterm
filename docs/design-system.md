...
| 3 | `styles/base.css` | 3,024 | Element defaults: reset, inherited type, buttons and inputs, focus rings, reduced motion. |
| 4 | `styles/chrome.css` | 9,094 | Application shell: the `#app` grid, top bar, workspace and session tabs, navigation rail, status bar, and the state classes on `.app-shell`. |
| 5 | `styles/hosts.css` | 10,780 | Hosts dashboard: page header, search row, toolbar, and the table and card shapes a saved host renders in. |
| 6 | `styles/inspector.css` | 7,964 | Connection form: the docked editor shell and its header, sections, fields and footer, plus the shared controls the keychain editor reuses. |
| 7 | `styles/keychain.css` | 9,062 | Keychain library and editor over the shared host-table language. |
| 8 | `styles/terminal.css` | 10,899 | Terminal surface: the xterm pane and its overrides, the session toolbar, the SFTP drawer. |
| 9 | `styles/states.css` | 8,870 | Nothing-to-show and something-went-wrong: empty placeholder, shortcuts dialog, connection-failure page. |

The size column is a working-tree measurement taken on Windows, and it is labelled that way because it does not reproduce everywhere: `core.autocrlf` is `true` and there is no `.gitattributes`, so the committed blobs are LF while seven of these nine files carry CRLF on disk. A checkout that keeps the blob form reads each of those seven smaller by exactly its line count — `tokens.css` 175, `base.css` 73, `chrome.css` 196, `hosts.css` 125, `inspector.css` 99, `keychain.css` 105, `states.css` 96 — while `fonts.css` and `terminal.css` are stored with LF endings already and read identically. `style.css` itself is CRLF and 1,363 bytes on disk against a 1,340-byte blob. Treat the numbers as an indication of how much each role carries, not as a checksum.
...
`[data-density="compact"]` is an orthogonal dimension to the theme and remaps one token: `--row-h: var(--row-h-compact)`. For that remap to reach a library row, the row's own button has to stop flooring it: `base.css` gives every `button` a `min-height: 38px`, which is exactly the comfortable `--row-h`, so a `.host-main` or `.keychain-card-main` inside a table row held its height no matter what the token said. Both are un-floored in table view, where the row's box is the target and the button merely fills it.
...
This is a census rather than a promise: the seven content partials hold 474 `var()` references, `tokens.css` holds 3 more inside its own derived values, and `terminal-view.ts` makes 6 `read()` calls. What is wired and what the screens still owe:
...
- Eleven tokens are still unreachable by anything on screen: `--overlay-soft`, `--overlay-press` and `--ac-focus` among the colours, and among the metrics three spacing steps (`--s-1`, `--s-5`, `--s-6`), three z-index stops (`--z-drawer`, `--z-popover`, `--z-dialog`) and two durations (`--t-1`, `--t-3`). They are verified by `design-tokens.test.mjs` in isolation rather than by a rendered pixel, which is the intended state of a ladder waiting for the screens — but it means a mistake in one of those eleven values is a mistake the browser has not yet contradicted.
