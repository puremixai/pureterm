...
- 新增 24px 状态栏，作为 `#app` 的第三栅格行：连接点与状态、在线时长、`user@host:port`、协商出的加密算法与主机密钥类型、终端尺寸、SFTP 速率、能力状态、版本号。只有连接状态元素带 `role="status"`。
  - **2026-09-24 执行前更正：** 状态栏只承载渲染层真正收到的东西 —— 连接状态、`user@host:port`、终端 `cols×rows`、版本号，旁边再配一个页面本就知道的工作区标签。在线时长、协商加密算法与远端主机密钥类型在任何地方都拿不到：Host 对一个会话只报告 `{id, host, port, username}`，从不读取 ssh2 的协商状态；SFTP 传输是整文件 RPC，没有进度事件。把它们暴露出来是一个 `@pureterm/protocol` 改动，需要自己的计划，不是 UI 任务，所以这些位置保持空缺而不是近似填充。
