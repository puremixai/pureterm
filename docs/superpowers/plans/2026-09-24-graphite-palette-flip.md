...
**Task 7 was not completed.** A visual review needs eyes on a rendered window and this session could not produce one: the in-app browser reported `visibilityState=hidden` on a `0x0` viewport, so no screenshot of any screen was available, and no Electron window was opened for inspection. What was checked is narrower than Step 2's seven items, and is reported as such:

- Against the running `npm run start:web` page, the built sheet references 50 distinct tokens through `var()` and every one of them resolves on the live document. The only unresolved custom property in the page belongs to the embedding environment, not to PureTerm.
- Four elements were read through `getComputedStyle` and match the ramp: `.app-topbar` `rgb(14, 16, 19)` is `--c-chrome`; `.host-avatar` `rgb(33, 37, 43)` is `--c-control` under an `rgb(242, 243, 245)` legend, which is `--tx-1` and is the `--legacy-on-solid` mapping live; `.nav-item` `rgba(90, 174, 255, 0.12)` is `--ac-bg`; `#terminal` `rgb(8, 9, 10)` is `--term-bg`. `box-shadow` and `transform` are `none` on all four.
- No SSH session was opened, so the terminal seam — where the DOM renderer's ground meets `#terminal`'s background, and where `.terminal-pane` padding shows through — was not exercised at all.

Items (a) through (g) stay open and become Plan 3's first checkpoint rather than a retuning now: a value changed under a green suite is how the register acquired its second set of hand-copied colours once already. Whoever runs it should look at item (c) first — the four host-avatar tones collapsing to one neutral ground is the only entry on that list that removes information rather than pigment.
