...
Tasks 1-6 landed as written, one commit each: `272de5a` the inspector token and grid track, `a87eebf` the docked editor, `c3c3826` the hosts table, `790c9e6` the keychain table and banner, `9c78652` the toggle language, `39caebe` the three breakpoints. Three deviations from the plan text, all of them the plan being wrong rather than the execution:

- The 630px and 1200px `@container` queries survived. The plan listed them among six breakpoints to collapse, but they guard the *card grid* inside `.keychain-main`, not the table, and a media query would key them to the viewport while the card column count belongs to the container. They are the only two container queries left in the cascade.
- The column-drop rules are scoped `#host-list:not(.card-view)` and `.keychain-list:not(.card-view)`. Without the `:not()` the 820px rule reaches into the card grid and overwrites the shape the card rules just gave it.
- Key→host association counts travel on a `client/host-counts` event emitted by `ClientHosts` after each refresh, not on an injected `ClientKeychain → ClientHosts` reference. The reverse edge already existed, so the direct injection would have been a cycle Cordis rejects at bootstrap.

**Task 7 found eight defects and fixed all eight.** None of them were visible to a green suite, and the first two were visible in the running app only.

- **The docked connection editor did not exist.** `.app-shell.inspector-open .app-body { grid-template-columns: … var(--insp-w) }` added a third track to a grid whose only children are the rail and `#main`. The form is a child of `#main`, so it never entered that track: it rendered at `y: 748` inside a 708px-tall column clipped by `overflow: hidden`, entirely below the fold, while the 322px track painted nothing but the page ground. Measured before the fix: `workspace: [52, 748, 706, 1039]`. The track moved to `#main`, which is the actual grid container, and the editor now resolves to `[758, 40, 322, 708]` — the same box the key editor takes, which is the claim `--insp-w` was introduced to make. Two guards now pin the structure instead of the intent: `#main` must be a grid, the track must be on `#main`, and an `--insp-w` track on `.app-body` fails.
- **The header row did not sit over its columns.** `#host-list` carried the `0 12px` inset while `#host-columns` was its sibling, so the two boxes differed by 9px and the `fr` units resolved against different widths. The inset moved to `#hosts-body`, and the header took transparent side borders so its border box matches a row's. Both tables now report identical `grid-template-columns` to the third decimal and `aligned: true`.
- **The hosts avatar was still the card's.** `.host-avatar` was 59px in the base rule, so every table row measured 61px — taller than the 38px `--row-h` it names. The base is now a 20px chip at `--r-1`, and card view sets the 58px square.
- **…and the card's avatar had the chip's corners.** Card view overrode the size but not the radius, so a 59px square carried `--r-1`'s 3px next to a keychain card's 58px at `--r-4`. Both are now 58px at `--r-4`; the keychain card's own padding (`13px 8px 13px 15px`) was likewise a two-pixel-drift version of the host card's and is now identical.
- **The keychain avatar sizes were inverted by Task 5.** `58px` lived on the base rule and `40px` on `.card-view`, written when the card was the only shape. After the table became the default every key row came out 41px tall beside a 39px host row. Base is now 26px at `--r-2` — the icon needs that much where the hosts table's `SSH` text chip gets by with 20px — and the card has the 58px square.
- **The key table said its type twice.** `.keychain-card-sub` printed `Type ed25519` under the name while the 类型 column next to it printed `ed25519`, and the second line is what made that row two lines tall. The sub-line is now table-hidden and card-visible, which is the same arrangement the hosts card uses for the address.
- **The column headers were illegible.** `.host-columns` set `color: var(--tx-4)`: 2.88:1 on the dark canvas and 2.61:1 on the light one, on 11px uppercase text. `--tx-4` is documented as a non-AA rung and the legality tables already said so; this is the second time in two review passes that a chrome control reached for it (Plan 3 caught the rail icons at 2.28:1). It is now `--tx-3`, measured 5.16:1 dark and 4.55:1 light, and `stylesheet-contract.test.mjs` refuses a `color: var(--tx-4)` on any selector that is not a `::placeholder`.
- **The density switch moved a token and no geometry.** `base.css` gives every `button` a `min-height: 38px`, which is exactly the comfortable `--row-h`, so the row's own button floored the row and compact did nothing. Measured: 39px in both modes before, 39px comfortable and 30px compact after un-flooring `.host-main` and `.keychain-card-main` in table view. This is the same property pair that beat `.nav-item { height: 34px }` in Plan 3.

Two smaller things found on the same pass: a column header rendered above an empty table framed nothing, so both headers now hide through `:has(#…-empty:not([hidden]))` rather than a second copy of the count; and the authentication column printed the protocol's own vocabulary — `password`, `private key · 本机文件` — in an otherwise Chinese table, so it now reads 密码 / 密钥库 · ed25519 / 本机文件, with the algorithm name kept because it is a proper noun.

**What could not be checked, and why that matters more than the list above.** Screenshots were unavailable again: the in-app surface reports `visibilityState=hidden`, so every colour value here was read through `getComputedStyle` after injecting `transition: none`, and the light theme is still numbers rather than a human impression. The narrow blocks are **unmeasured**: the in-app viewport is fixed at 1080px, and the iframe harness that worked against the static prototype cannot load the running app, because the loopback host refuses to be framed (`X-Frame-Options`/CSP — correct behaviour, and the reason the 1100/900/820/620 rules are verified only by cascade reasoning). In particular the new `max-width: 900px` editor overlay is unverified in a browser, and below 620px its `position: absolute; inset: 0` resolves against a `#main` whose height is content-driven, so a phone-width drawer will size to the list behind it rather than to the viewport. That is recorded as a gap, not fixed blind. No key was ever created against the running Web host in this environment — the save produced no row and logged nothing — so the keychain table was measured on the draft row, which shares the same six tracks but does not exercise the fingerprint or association cells.

`npm run verify` and `npm run verify:electron` both exit 0, 54 UI guards pass with the four new assertions, and the lifecycle suite's host-table checks now assert the six tracks and the authentication wording rather than trusting a screenshot.
