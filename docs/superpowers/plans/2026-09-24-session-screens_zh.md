...
任务 1-8 以九个 commit 落地：`4ec122d` 两栏、`d8bd385` 握把与时长阶梯、`c30db3c` 文件表格、`536cec0` 面包屑、`ab1eaab` 与 `16caff0` 分诊表及其清点、`5db90c9` 四节点路线、`995c13d` 通知、`cc4efab` 四种状态与那道结构守卫，另有 `5e99898` 把本计划与实测结果对齐。四处偏离计划文本，四处都是计划写错而不是执行走样：

- **`client/split-change` 事件被丢掉。** 比例由同一个服务写、由同一个服务读；`session-change` 与 `tab-closed` 本来就汇入 `sync()`。向唯一的产生者广播它自己的值，只是给后来人多一件要追的事。
- **820px 那条堆叠规则计划里根本没有。** 在 447px 视口上量出来的是：两栏各自落到自己的下界 —— 447px 的窗口里塞着 465px 的内容。删掉行布局就得为规格留给它的那个宽度给出替代。
- **180ms 归到 `--t-2`，不是 `--t-3`。** 计划写的是 240ms；而 180 离 `--t-2` 差 20ms、离 `--t-3` 差 60ms，取两档会让主机卡片与密钥卡片的过渡分到两档上。
- **文件表需要一个计划没预料到的容器查询。** 大小、模式、修改时间合占 254px 固定轨道，而握把的夹持允许面板收窄到 220px 下界。

**发现并修好六处缺陷，其中两处只有量能到。**

- 写成 `min-height: 28px` 的文件行实际渲染 **39px**，因为 `base.css` 给每个 `button` 托了 38px 下限。这是本轮改版里这对属性第三次赢过写下的数值（计划三的导航项、计划四的密度开关），现在同样有守卫钉住。
- 失败日志有一条涂 `.failure-log-entry i` 的规则 —— 而日志从不生成那个元素。**错误行从来没有被涂过色。** 现在每行真的带标记，那条规则于是第一次生效，失败行取 `--err`（在 inset 底上浅色 5.09:1、暗色 5.90:1）。
- 日志行号本要用 `--tx-3`，而它在 `--c-inset` 上只有 **4.16:1**；改用 `--tx-2`。这是发出去之前算出来的，不是之后。
- 失败页头像是**第三种**尺寸（60px），还带着 20px 徽标的圆角。现在是 58px `--r-4`，与两张库卡片一致。
- **`999px` 写了两处**，不是一处：路线节点之外，还有已发布外壳里的 `.tab-state-dot`。新加的 `doesNotMatch` 守卫第一次跑就抓到第二处。
- 画出的四个节点索引的是一个七元阶段表。没有 `NODE_OF` 的话，TCP 失败会点亮「密钥交换」节点。这一条在本计划的评审里被抓住，代码还没写。

**最有教益的发现不是界面缺陷，而是守卫上的一个洞。** 我在插入通知样式时把 `.empty { margin: 12px;` 替换掉了，而不是接在它前面。结果是深度为零处裸着一条声明：esbuild 原样发出，浏览器直接丢弃，而 **60 条守卫在一个已经静默不存在的规则上保持全绿。** `stylesheet-contract.test.mjs` 现在会读每个分片的括号是否配平、有没有声明落在块外，并且是拿那次真实的破坏验过的 —— 它的第一版按行计数花括号，在本代码库「一行一规则」的写法上误报，所以最终版只问那个能区分故障的问题。找出三条被大意改写的宿主文案的那个清点脚本，也同样成了常驻测试而不是一次性工具。

**查不了的部分，以及它为什么更要紧。** 截图全程不可用（`visibilityState=hidden`），因此本文每个颜色与几何值都是 `getComputedStyle` 读出的，浅色那批读数之前先关掉了过渡。窄屏是靠强制容器宽度加上内置浏览器恰好给出的视口探的（这一整段会话里它从 1080px 漂到 354px），从未靠真的调整窗口；100% 与 125% 缩放下的桌面窗口仍未测过。**这个环境里始终没有打开过一次真实的 SSH 会话**，所以分栏、握把、文件表与面包屑是拿合成行在真实级联上量的，路线只在假 API 的渲染器套件里验过 —— 那确实走真实 DOM 与真实事件路径，但不是真服务器。`smoke-client-lifecycle.mjs` 会剥掉样式表的 `<link>`，几何断言在那儿根本不可能成立；文件表测试的早期版本用 `getComputedStyle` 断言列对齐，而它唯一可能的读数是没有样式时的默认值。密钥库列表在这个环境里从未真正得到一把密钥，所以它的表格仍是在草稿行上量的。

`npm run verify` 与 `npm run verify:electron` 均退出 0；61 条 UI 守卫通过，新增断言横跨 `visual-contract`、`stylesheet-contract`、`design-tokens`、`failure-diagnostics` 与 `host-message-census` 五个文件，并且跑了四次反向验证以确认新守卫确实能变红。
