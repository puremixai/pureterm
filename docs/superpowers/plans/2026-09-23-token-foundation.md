...
Also for Plan 2: `legacy.css` has no light group, so all 100 entries stay dark in light mode — correct while the file exists, and a one-file change when it does not. And named colours remain invisible to the literal regex, though nothing is being missed today: the claim that `white` survives as a colour in three places is false. No partial carries a named colour as a value; all 13 occurrences of the word `white` in the partials belong to a `white-space` property, and `transparent` is used deliberately for borders and fills. `\bwhite\b` as a pattern also matches `white-space`, so catching a future named colour needs a value-position-aware rule rather than a wider word list.
...
The closing review added two more, both of which are now assertions rather than advice:

7. **Seven ANSI entries are hand-copies of live tokens, and are now tied.** Entry 1 mirrors `--err`, 2 `--ok`, 3 `--warn`, 4 `--ac`, 7 `--tx-2`, 12 `--ac-hi` and 15 `--tx-1`, each byte-identical today, and none reached by a `var()`. `terminal-theme.test.mjs` now reads the `ANSI` array out of `terminal-view.ts` — it does not re-type the literals, which could certify a drifted array — and asserts those seven triplets still equal the **dark** group's tokens, on the same reasoning `design-tokens.test.mjs` already gives for `--term-cursor`: without the tie a palette flip leaves the copy behind in silence. So retuning any of those seven in Plan 2 fails until the ANSI list moves with it. Do not re-tune by hand in one file, and do not "fix" the failure by moving the tie to the light group: the list is dark terminal material in both themes.
8. **Reference direction is now checked, and the flip is what stresses it.** `stylesheet-contract.test.mjs` asserts every `var(--x)` in any partial — the two registers included, since a token value may point at a name too — names a token declared in `tokens.css` or `legacy.css`. Until now the suite only checked the opposite direction, that a declaration has a call site. The gap mattered because an unknown custom property is not an error: it is substituted at computed-value time, so `padding: var(--s-3x)` builds, ships and simply stops applying, with nothing failing anywhere. Every one of the new ramp's roughly 90 call sites is still unwritten, so the flip creates the whole exposure in one commit, and a transposed suffix is the likeliest defect of a mechanical rewrite. A failing line in that report is a name to declare or correct, never a reason to widen the exemption pair.
