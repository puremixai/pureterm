...
- **任务 3 既不要就地加前缀，也不要把登记表留在 `tokens.css` 里。** 它 24 条里只有四条自称是债，另外二十条读起来像永久 token，于是配色翻转得依赖一份只存在于过期文档里的删除清单。给 24 条全加 `--legacy-*` 固然能解决，代价是在约 102 处字面量替换之上再叠约 90 处机械引用改写 —— 为改而改。改为：任务 3 把整块登记表移进 `packages/ui/src/styles/legacy.css`，由清单在 `tokens.css` 之后紧接着引入。于是 `tokens.css` 只剩新体系，`var(--nav)` 之类原样继续工作，禁止字面量的测试排除两个文件而不是一个，计划二的删除变成一次 `git rm` 加一行清单。这推翻本文档早前对独立 `legacy.css` 的否决：当初否决靠的是 `block(':root')` 解析隐患，而任务 1 的加固已经关掉它，且文件给出的边界是机器可读的，不只是"写在文档里"。不过这里当初把发丝线那几个名字说错了：`--line`、`--line-soft`、`--line-strong` 没有一个住在 `legacy.css` —— 三条全部声明在 `tokens.css` 里，各自引用数为零，所以它们是待接线的阶梯。今天屏幕上真正在用的是 `legacy.css` 自己那道七档阶梯（`--legacy-line-faint` .05、`--legacy-line-hair` .07、`--legacy-line-edge` .08、`--legacy-line` .085、`--legacy-line-mid` .12、`--legacy-line-field` .13、`--legacy-line-strong` .17）。于是 `--line-strong` 与 `--legacy-line-strong` 是两个平行名字对应两个不同取值，一个待接线、一个活着，翻转必须按实测取值配对，而不是按名字。
...
计划二还需知道：`legacy.css` 没有浅色组，所以 100 条在浅色模式下全是深色 —— 文件活着时这是对的，文件没了就是一处单行改动。另外具名颜色对字面量正则依然不可见，但今天并没有漏掉什么："`white` 作为颜色还留在三处"这句话是错的。任何分片都没有把具名颜色当作取值使用；分片里 `white` 这个词出现 13 次，每一次都属于某个 `white-space` 属性，而 `transparent` 是被刻意用在描边与填充上的。`\bwhite\b` 作为匹配式同样会命中 `white-space`，所以要捕获未来出现的具名颜色，需要的是一条能识别取值位置的规则，而不是更宽的词表。
...
收尾评审又添两条，且两条如今都是断言，不再是建议：

7. **七项 ANSI 是活 token 的手工拷贝，如今已被拴住。** 第 1 项镜像 `--err`，第 2 项 `--ok`，第 3 项 `--warn`，第 4 项 `--ac`，第 7 项 `--tx-2`，第 12 项 `--ac-hi`，第 15 项 `--tx-1`，今天逐字节相同，而没有任何 `var()` 抵达它们。`terminal-theme.test.mjs` 现在从 `terminal-view.ts` 里读出那张 `ANSI` 数组 —— 它没有把字面量重敲一遍，那会为一份已经漂移的数组作证 —— 并断言这七个三分量仍然等于**深色**组的对应 token，依据就是 `design-tokens.test.mjs` 为 `--term-cursor` 给出的同一条理由：没有这根连线，配色翻转会把副本留在原地，无声无息。所以在计划二里重调这七个中的任何一个，都会失败到 ANSI 列表跟着改动为止。不要只在单个文件里手工重调，也不要把连线挪到浅色组来"修好"失败：这张列表在两个主题里都是深色终端材料。
8. **引用方向现在被检查了，而翻转正是压力来源。** `stylesheet-contract.test.mjs` 断言任何分片里的每一个 `var(--x)` —— 两个登记表本身也包括，因为 token 取值同样可能指向一个名字 —— 都必须指向某个在 `tokens.css` 或 `legacy.css` 里声明过的 token。在此之前测试套件只检查反方向，即一条声明有没有调用点。这个缺口要紧，是因为未知的自定义属性并不是错误：它在计算值阶段被替换，于是 `padding: var(--s-3x)` 能构建、能发布，只是从此不再生效，而任何地方都不会失败。新灰阶那约 90 处调用点至今一处都还没写上，所以翻转会在单个提交里制造出全部暴露面，而一次机械改写最容易出的缺陷就是一个字母顺序写反的后缀。那份报告里的一条失败是要去声明或改正一个名字，绝不是放宽豁免对的理由。
